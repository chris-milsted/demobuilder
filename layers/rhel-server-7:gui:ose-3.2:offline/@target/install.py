...
def download_imagestream_images(step, steps, repo):
    ...
    for i, istr in enumerate(istrs, 1):
        ...
        for t in istr.spec.tags:
            ...
            t["importPolicy"] = k8s.API.decode({"insecure": True})
...
def download_imagestream_git_repos(step, steps):
    ...
    for i, istr in enumerate(istrs, 1):
        print >>sys.stderr, "STATUS: %d/%d: %d/%d" % (step, steps, i, len(istrs))
        for t in istr.spec.tags:
            if "annotations" in t and t.annotations is not None and "sampleRepo" in t.annotations:
    ...
    for i, istr in enumerate(istrs, 1):
        ...
        for t in istr.spec.tags:
            if "annotations" in t and t.annotations is not None and "sampleRepo" in t.annotations:
                ...
                if istr.metadata.name == "jboss-eap70-openshift" and t.name == "1.3-Beta":
                    continue
...
def reconfigure_master():
    ...
    system("systemctl restart atomic-openshift-master")
...
def main():
    ...
    download_imagestream_images(1, 5, ep)
    download_template_git_repos(2, 5)
    download_imagestream_git_repos(3, 5)
    do_template_builds(4, 5)
    do_imagestream_builds(5, 5)
    remove_images()

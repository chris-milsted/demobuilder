...
# POD=$(oc process logging-deployer-template -n openshift -v KIBANA_HOSTNAME=kibana.example.com,ES_CLUSTER_SIZE=1,PUBLIC_MASTER_URL=https://openshift.example.com:8443,IMAGE_PREFIX=brew-pulp-docker01.web.prod.ext.phx2.redhat.com:8888/openshift3/ | oc create -f - | cut -d '"' -f2)
...
# for is in logging-auth-proxy logging-elasticsearch logging-fluentd logging-kibana; do
#   while ! oc import-image --insecure --all $is; do
#     sleep 1
#   done
# done
